...
fzf-pacman() {
  ...
  _remove() {
    ...
    echo "$pkgs" | xargs sudo pacman -Rns
  }
  ...
  _aur() {
    ...
    install)
      local pkgs
      pkgs=$($helper -Slq | _fzf_base "AUR Install ❯" --preview "$helper -Si {1}")
      [[ -z "$pkgs" ]] && _no_selection && return
      ...
      echo "$pkgs" | xargs $helper -S
      ;;
    remove)
      local pkgs
      pkgs=$(pacman -Qqm | _fzf_base "AUR Remove ❯" --preview "pacman -Qi {1}")
      [[ -z "$pkgs" ]] && _no_selection && return
